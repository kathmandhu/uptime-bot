            ...
            if (interaction.customId === 'eval-modal') {
                ...
                try {
                    ...
                    if (evaled.length < 800) {
                        interaction.reply({
                            content: `${client.emotes.MESSAGE.y} **Code Output:**\`\`\`xl\n${evaled}\n\`\`\``,
                        });
                    } else {
                        hastebin(evaled, 'js').then(r => {
                            const row = new ActionRowBuilder().addComponents([
                                new ButtonBuilder().setLabel('Hastebin').setURL(r).setStyle(5),
                            ]);
                            interaction.reply({
                                content: `${client.emotes.MESSAGE.y} **Hastebin Output:**`,
                                components: [row],
                            });
                        });
                    }
                                ...
                                interaction.editReply({
                                    embeds: [
                                        client
                                            .Embed(false)
                                            .setDescription(
                                                client.emotes.MESSAGE.i +
                                                    " The Project you're Trying To Re ster Is Already In The Database"
                                            ),
                                    ],
                                });
                            }, 2000);
