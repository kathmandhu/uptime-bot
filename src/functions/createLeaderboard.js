...
module.exports = async (client, interaction, title, lb, ephemeral = false) => {
    ...
    await interaction.reply({
        embeds: [generateEmbed(0, 0, lb, title, page)],
        ephemeral,
        fetchReply: true,
    });
    if (lb.length <= 10) return;
    let button1 = new ButtonBuilder().setCustomId('start').setStyle(2).setEmoji(`991378355473633300`).setDisabled(true);
    let button2 = new ButtonBuilder()
        .setCustomId('back_button')
        .setEmoji('991359218970001428')
        .setStyle(2)
        .setDisabled(true);
    let button3 = new ButtonBuilder().setCustomId('forward_button').setEmoji('991359213819396126').setStyle(2);
    let button4 = new ButtonBuilder().setCustomId('end').setEmoji(`991378371441340506`).setStyle(2);
    ...
    let currentIndex = 0;
    const collector = message.createMessageComponentCollector(button => button.user.id === interaction.user.id, {
        time: 60000,
    });
    ...
    collector.on('collect', async btn => {
        if (btn.user.id == interaction.user.id) {
            ...
            interaction.editReply({
                fetchReply: true,
                embeds: [generateEmbed(currentIndex, currentIndex, lb, title, page)],
                components: [row2],
            });
            btn.deferUpdate();
        }
