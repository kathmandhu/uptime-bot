    ...
    client.on("interactionCreate", async (interaction) => {
      ...
      try {
          ...
          return await interaction
            .reply({
              ...
              embeds: [
                new EmbedBuilder()
                  .setTitle(
                    `${emojis.MESSAGE.x} **That Command Has Been Disabled By The Developers! Please Try Later.**`
                  )
            ...
            return await interaction
              .reply({
                ...
                embeds: [
                  new EmbedBuilder()
                    .setDescription(
                      `${emojis.MESSAGE.x} **You cannot use \`${slashCommand.name}\` command as this is a developer command.**`
                    )
                    .setColor(client.embed.wrongcolor),
                ],
              })
        ...
        if (slashCommand.guildOnly) {
          const private = client.config.SERVER.OFFICIAL.Guild_ID_1.concat(
            client.config.SERVER.Guild_ID_2
          );
          if (!private.includes(interaction.guild.id)) {
            return interaction.reply({
              ...
              embeds: [
                new EmbedBuilder()
                  .setTitle(
                    `${emojis.MESSAGE.x} ${interaction.user.username} You have entered an invalid command!`
                  )
                  .setDescription(
                    `The command \`${slashCommand.name}\` can only be used in the official server.`
                  )
                  .setColor(client.embed.wrongcolor),
              ],
            });
        ...
        if (slashCommand.nsfwOnly && !interaction.channel.nsfw) {
          return interaction.reply({
            ...
            embeds: [
              new EmbedBuilder()
                .setDescription(
                  `${emojis.MESSAGE.x} This command can only be used in NSFW channels!`
                )
                .setColor(client.embed.wrongcolor),
            ],
          });
          ...
          if (
            ...
            const userPerms = new EmbedBuilder()
              .setDescription(
                `${emojis.MESSAGE.x} ${
                  interaction.user
                }, You don't have ${parsePermissions(
        ...
        if (slashCommand.cooldown && slash(interaction, slashCommand)) {
          return interaction.reply({
            ...
            embeds: [
              new EmbedBuilder()
                .setTitle(
                  `${emojis.MESSAGE.x} You have been cooldown for \`${slashCommand.cooldown}\` seconds!`
                )
                .setDescription(
                  `Please wait \`${slash(interaction, slashCommand).toFixed(
                    1
                  )}\` Before using the \`${slashCommand.name}\` command again!`
                )
                .setColor(client.embed.wrongcolor),
            ],
          });
        ...
        if (client.config.CHANNELS.COMMANDS_LOGS && set.COMMANDS_LOGS)
          await client.channels.cache
            .get(client.config.CHANNELS.COMMANDS_LOGS)
            .send({
              embeds: [
                new EmbedBuilder()
                  ...
                  .setAuthor({
                    name: "Slash Command",
                    iconURL: `https://cdn.discordapp.com/emojis/942758826904551464.webp?size=28&quality=lossless`,
                  })
