          ...
          if (!validUrl.isUri(url)) {
            return setTimeout(() => {
              interaction.editReply({
                embeds: [],
                content:
                  client.emotes.MESSAGE.x + " Please provide a vaild url",
              });
            }, 2000);
          }
          urlsConfig.findOne(
            ...
            async function (err, data) {
              if (!data) {
                return interaction.editReply({
                  embeds: [
                    client
                      .Embed(false)
                      .setDescription(
                        client.emotes.MESSAGE.i + " Project is not Registered!"
                      ),
                  ],
                });
              } else {
                urlsConfig
                  ...
                  .then(async () => {
                    let new_pros = await client.projects.filter(
                      (p) => p !== url
                    );
                    ...
                    let embed = client
                      .Embed(false)
                      .setDescription(
                        client.emotes.MESSAGE.y + " Removed Succesfully!"
                      );
            ...
            async function (err, data) {
              if (!data || data.length === 0) {
                return interaction.editReply({
                  embeds: [
                    client
                      .Embed(false)
                      .setDescription(
                        client.emotes.MESSAGE.i + " You don't have any project"
                      ),
                  ],
                });
              } else {
                ...
                let embed = client
                  .Embed()
                  .setTitle(
                    "Select what project you want remove from being uptimed."
                  );
                ...
                if (content.length > 0 && content.length < 2)
                  num_btns.push(btn_num1);
                else if (content.length > 1 && content.length < 3)
                  num_btns.push(btn_num1, btn_num2);
                else if (content.length > 2 && content.length < 4)
                  num_btns.push(btn_num1, btn_num2, btn_num3);
                else if (content.length > 3 && content.length < 5)
                  num_btns.push(btn_num1, btn_num2, btn_num3, btn_num4);
                else
                  num_btns.push(
                    btn_num1,
                    btn_num2,
                ...
                collector.on("collect", async (b) => {
                  ...
                  if (b.customId === "back") {
                    if (currentPage !== 0) {
                      ...
                      let new_array = [];
                      if (content.length > 0 && content.length < 2)
                        new_array.push(btn_num1);
                      else if (content.length > 1 && content.length < 3)
                        new_array.push(btn_num1, btn_num2);
                      else if (content.length > 2 && content.length < 4)
                        new_array.push(btn_num1, btn_num2, btn_num3);
                      else if (content.length > 3 && content.length < 5)
                        new_array.push(btn_num1, btn_num2, btn_num3, btn_num4);
                      else
                        new_array.push(
                          btn_num1,
                          btn_num2,
              ...
              return interaction.reply({
                ...
                embeds: [
                  client
                    .Embed(false)
                    .setDescription(
                      client.emotes.MESSAGE.i + " You dont have any project."
                    ),
                ],
              });
            require(`${process.cwd()}/src/functions/createLeaderboard.js`)(
              client,
              interaction,
              "・Projects",
              lb,
